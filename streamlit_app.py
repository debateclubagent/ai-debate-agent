...
def build_yellow_prompt(question, previous_rounds, votes):
    ...
    for i in range(len(votes) // 4):
        if votes.get(f"like_yellow_{i}"):
            vote_summary.append(f"你在第{i+1}轮支持了黄帽观点")
        if votes.get(f"dislike_yellow_{i}"):
            vote_summary.append(f"你在第{i+1}轮反对了黄帽观点")
    vote_hint = "
".join(vote_summary)
    if previous_rounds:
...
def build_black_prompt(question, yellow_viewpoint, previous_rounds, votes):
    ...
    vote_summary = []
    for i in range(len(votes) // 4):
        if votes.get(f"like_black_{i}"):
            vote_summary.append(f"你在第{i+1}轮支持了黑帽观点")
        if votes.get(f"dislike_black_{i}"):
            vote_summary.append(f"你在第{i+1}轮反对了黑帽观点")
        if votes.get(f"like_yellow_{i}"):
            vote_summary.append(f"你在第{i+1}轮更倾向黄帽")
    vote_hint = "
".join(vote_summary)
    if previous_rounds:
